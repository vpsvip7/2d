...
if [[ ! $(id -u) = 0 ]]; then
  ...
  msg -bar
  print_center -ama "DEBE EJECUTAR DESDE EL USUSRIO ROOT"
  msg -bar
  CTRL_C
fi
...
do
  case $1 in
    -s|--start)install_start; post_reboot; time_reboot "7";;
...
title -ama '[Proyect by vps200]'
print_center -ama 'INSTALANDO SCRIPT '
sleep 2; del 1
...
print_center -verd 'Instalacion OK '
sleep 2s
rm $HOME/lista-arq
[[ -d ${SCPinstal} ]] && rm -rf ${SCPinstal}
rm -rf /usr/bin/menu
